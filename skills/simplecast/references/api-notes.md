...
| `GET /analytics/embed/listens` | `podcast` **or** `episode` (required) | `start_date`, `end_date`, `interval`, `sort`, `email` | Embed listen counts. |
| `GET /analytics/embed/locations` | `podcast` **or** `episode` (required) | `start_date`, `end_date`, `limit`, `offset`, `sort`, `email` | Embed geographic breakdown. |
| `GET /analytics/embed/speeds` | `podcast` **or** `episode` (required) | `start_date`, `end_date`, `email` | Playback speed distribution (0.5x–2.0x). Collection documents **both** `podcast` and `episode` as valid scope params — see contradiction note below. |
...
The Simplecast API documents more query parameters per endpoint than this server's 25 tools currently expose as typed arguments (e.g. `interval`, `sort`, `email`, `country`, `state`, and the `episodes` comma-list filter shown above). Every tool accepts the params called out in the README's tool table; anything documented above but not in that table is reachable today via the `simplecast_get` escape hatch (pass the endpoint's `path` and the extra params under `query`).

One specific note: the collection documents **both** `podcast` and `episode` as valid scope params on `GET /analytics/embed/speeds`, even though `get_embed_analytics` treats `report: "speeds"` as podcast-only and will reject an `episode_id`. If you need episode-scoped speed data, call `simplecast_get` directly against `/analytics/embed/speeds` with `episode` in `query`.
